...
function onOpen(e) {
  try {
    ...
    console.warn(
      "Note: getUi() cannot be called when running directly from the Apps Script editor.",
    );
  }
}
...
function onHomepage(e) {
  ...
  var card = CardService.newCardBuilder().setHeader(
    CardService.newCardHeader()
      .setTitle("SheetsAPI")
      .setSubtitle("Turn this sheet into a REST API"),
  );
  ...
  section.addWidget(
    CardService.newTextParagraph().setText(
      "<b>Spreadsheet:</b> " +
        (spreadsheet ? spreadsheet.getName() : "(open a sheet)"),
    ),
  );
