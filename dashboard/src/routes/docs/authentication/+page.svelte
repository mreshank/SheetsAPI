...
>
  ...
  <ol class="mt-2 list-decimal pl-5 space-y-1">
    <li>
      <strong>You ↔ Google</strong> - OAuth 2.0 authorization code flow. You consent
      once; we store an encrypted refresh token.
    </li>
    <li>
      <strong>Your callers ↔ SheetsAPI</strong> - optional Bearer API keys on every
      request.
    </li>
  </ol>
  ...
  <p>
    From the dashboard, click "Sign in with Google". You'll be redirected to
    Google's consent screen, then back to your dashboard. We request:
  </p>
  ...
  <p>
    Endpoints are <strong>public by default</strong>. The moment you create your
    first API key in the dashboard, <em>all</em> your endpoints start requiring it.
  </p>
  ...
  <ol class="mt-2 list-decimal pl-5 space-y-1">
    <li>
      Sign out of the SheetsAPI dashboard - this deletes all our records for
      your account.
    </li>
    <li>
      Remove the app from <a
  ...
  <p>
    Refresh tokens are AES-GCM-256 encrypted at rest with a key stored
    separately from the database. See <a
      href="/security"
      style="color:rgba(255,255,255,0.7);text-decoration:underline;">Security</a
    >.
  </p>
</DocLayout>
