...
<\/script>`;
  ...
  const faqs = [
    {
      q: "Will spam bots flood my sheet?",
      a: "Likely yes if you post publicly. Add a honeypot field, Cloudflare Turnstile, or require an API key. SheetsAPI endpoints require a Bearer key once you create one in the dashboard.",
    },
    {
      q: "Can I trigger an email on submission?",
      a: "Yes - attach a Google Apps Script onEdit trigger to your sheet to send via MailApp, or use Make/Zapier (supported in v0.4).",
    },
    {
      q: "How many submissions per day?",
      a: "Google Sheets quota is 300 writes/min/project. For a busy form, consider batching or upgrading to a Pro tier (v0.8).",
    },
  ];
</script>
